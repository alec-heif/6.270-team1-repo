...
uint8_t is_opponent_in_our_path(int curr_sextant, int target_sextant, int direction){
    uint8_t returned = FALSE;
    uint8_t opp_sextant;
    switch(direction) {
        case COUNTER_CLOCKWISE:
            if(target_sextant < curr_sextant){
            ...
            opp_sextant = get_current_sextant(field_state.opponent_loc);
            for(int sex = curr_sextant; sex <= target_sextant; sex++){
                if(mod_ui(sex, 6) == opp_sextant)
                    returned = TRUE;
            }
            break;
        case CLOCKWISE:
...
int get_waypoint_direction_five_param(Location curr_loc, Location target_loc, uint8_t curr_sextant, uint8_t target_sextant, float curr_angle){
    ...
    if(is_opponent_in_our_path(curr_sextant, target_sextant, direction) && field_state.stage != FIRST_STAGE && get_current_sextant(field_state.opponent_loc) != curr_sextant)
...
float bound_waypoint_distance_float_param(float distance_increment, float current_distance){
    ...
    if(new_distance < (INNER_HEX_VERTEX_DIST + ROBOT_WIDTH/2 + 250))
        new_distance = INNER_HEX_VERTEX_DIST + ROBOT_WIDTH/2 + 250;
    if(new_distance > (OUTER_HEX_APATHEM_DIST - ROBOT_WIDTH/2 - 250))
        new_distance = OUTER_HEX_APATHEM_DIST - ROBOT_WIDTH/2 - 250;
    return new_distance;
}

float bound_waypoint_distance(float distance_increment, Location target_loc_waypoint){
    float new_distance = distance_increment + pythagorean(target_loc_waypoint.x, target_loc_waypoint.y);
    if(new_distance < (INNER_HEX_VERTEX_DIST + ROBOT_WIDTH/2 + 250))
        new_distance = INNER_HEX_VERTEX_DIST + ROBOT_WIDTH/2 + 250;
    if(new_distance > (OUTER_HEX_APATHEM_DIST - ROBOT_WIDTH/2 - 250))
        new_distance = OUTER_HEX_APATHEM_DIST - ROBOT_WIDTH/2 - 250;
...
float get_d(uint16_t encoder_value){
    return (((float)encoder_read(encoder_value)) / CLICKS_PER_INCH)*INCHES_TO_PIXELS;
}
...
void set_new_destination(Location curr_loc, Location target_loc){
    ...
    float degrees = mod_f(gyro_get_degrees(), 180);
    if(field_state.drive_direction == BACKWARD)
        degrees = mod_f(degrees + 180, 180);
    int direction = get_waypoint_direction(curr_loc, target_loc, mod_f(gyro_get_degrees(), 180));
    //if(field_state.stage == FIRST_STAGE)
        //direction = COUNTER_CLOCKWISE;
    ...
    switch(field_state.color){
        case BLUE:
            if(sextant == 3)
                ...
                int direction_to_3 = get_waypoint_direction(field_state.curr_loc_plus_delta, get_dump_location(3), mod_f(gyro_get_degrees(), 180));
                uint8_t distance_to_3 = get_sextant_difference(sextant, 3, direction_to_3);
                int direction_to_5 = get_waypoint_direction(field_state.curr_loc_plus_delta, get_dump_location(5), mod_f(gyro_get_degrees(), 180));
        ...
        case RED:
            if(sextant == 0)
                ...
                int direction_to_0 = get_waypoint_direction(field_state.curr_loc_plus_delta, get_dump_location(0), mod_f(gyro_get_degrees(), 180));
                uint8_t distance_to_0 = get_sextant_difference(sextant, 0, direction_to_0);
                int direction_to_2 = get_waypoint_direction(field_state.curr_loc_plus_delta, get_dump_location(2), mod_f(gyro_get_degrees(), 180));
...
void get_your_ass_to_a_toilet(){
    ...
    time_during_dump = DUMP_BASE_CASE + 50*field_state.balls_held;
}
